...
use dex_router_encoding::{MAX_SWAP_PATH, bytes_array_encoded_size, encode_bytes_array};
...
#[cfg_attr(
	not(test),
	pvm_contract_macros::contract("DexRouter.sol", allocator = "bump", allocator_size = 8192)
)]
mod dex_router {
	#[cfg(test)]
	...
	pub fn swap_exact_in(
		...
		let out =
			dex_call(&build_swap_exact_in(&path_refs, amount_in, amount_out_min, &caller, false));
	...
	pub fn swap_exact_out(
		...
		let path_refs: Vec<&[u8]> = path.iter().map(|p| p.0.as_slice()).collect();
		let out =
			dex_call(&build_swap_exact_out(&path_refs, amount_out, amount_in_max, &caller, false));
	...
	fn build_add_liquidity(
		...
		enc_two_bytes(
			...
			a1,
			a2,
			&[enc_u256(d1), enc_u256(d2), enc_u256(m1), enc_u256(m2), enc_addr(mint_to)],
		)
	}
	...
	fn build_remove_liquidity(
		...
		enc_two_bytes(&s, a1, a2, &[enc_u256(lp), enc_u256(m1), enc_u256(m2), enc_addr(to)])
	}
...
#[cfg(test)]
mod tests {
	use super::{Bytes, dex_router::*, mock_api};
	use alloc::{vec, vec::Vec};
	use ruint::aliases::U256;
	...
	fn add_liquidity_pulls_and_sweeps() {
		...
		let result =
			add_liquidity(a1, a2, U256::from(1000u64), U256::from(1000u64), U256::ZERO, U256::ZERO);
		assert!(result.is_ok());
	...
	fn remove_liquidity_calls_precompile() {
		...
		let a1 = b(vec![0x01, 1, 0, 0, 0]);
		let a2 = b(vec![0x01, 2, 0, 0, 0]);
		let result = remove_liquidity(a1, a2, U256::from(500u64), U256::ZERO, U256::ZERO);
	...
	fn remove_liquidity_reverts_on_failure() {
		...
		let a1 = b(vec![0x01, 1, 0, 0, 0]);
		let a2 = b(vec![0x01, 2, 0, 0, 0]);
		let _ = remove_liquidity(a1, a2, U256::from(500u64), U256::ZERO, U256::ZERO);
	}
	...
	fn remove_liquidity_returns_both_amounts() {
		...
		let a1 = b(vec![0x01, 1, 0, 0, 0]);
		let a2 = b(vec![0x01, 2, 0, 0, 0]);
		let result = remove_liquidity(a1, a2, U256::from(1000u64), U256::ZERO, U256::ZERO);
